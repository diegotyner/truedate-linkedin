...
import type { TrueDateMessage, ParsedJobData } from "./types.js";

const SHADOW_HOST_ID = "truedate-banner-root";
const jobCache = new Map<string, ParsedJobData>();
...
function renderBanner(data: ParsedJobData): void {
  ...
  const ageText =
    data.daysSinceOriginal === 0
      ? "This is the original posting"
      : `Originally posted ${data.daysSinceOriginal} day${data.daysSinceOriginal === 1 ? "" : "s"} before this listing`;
  ...
  container.innerHTML = `
    <div class="fixed top-4 right-4 z-[999999] bg-slate-900 text-white rounded-lg overflow-visible border border-slate-700 p-3 font-sans text-xs flex flex-col gap-1 max-w-xs transition-all">
      <div class="flex items-center justify-between gap-2">
        ...
        ${
          data.expireAt
            ? `<span class="pl-3 text-slate-400 font-mono text-[10px]">
  ...
  // box-shadow set via inline style, not Tailwind class. Tailwind shadow not working on this element, and this is a proven workaround.
  const cardEl = container.querySelector(".fixed") as HTMLElement | null;
  if (cardEl) {
    cardEl.style.boxShadow = "0 10px 25px -3px rgba(0,0,0,0.75)";
  }
}

function getCurrentJobIdFromUrl(): string | null {
  try {
...
function updateBannerFromCache(): void {
...
// Check for SPA URL changes periodically
...
window.addEventListener("message", (event: MessageEvent) => {
  ...
  const data = event.data as TrueDateMessage | undefined;
